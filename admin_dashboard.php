...
<head>
   ...
   <style>
    ...
    #sidebar { transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); min-width: 260px; background: linear-gradient(180deg, #001f3f 0%, #001021 100%); box-shadow: 4px 0 10px rgba(0, 0, 0, 0.1); }
...
<body class="text-gray-800">
        ...
        <aside id="sidebar" class="w-64 bg-primary text-white p-6 shadow-lg fixed md:relative inset-y-0 left-0 transform md:transform-none transition-transform duration-300 ease-in-out z-40">
            <div class="flex items-center justify-between mb-8">
               <div class="flex items-center">
                    <div class="h-10 w-10 bg-secondary rounded-lg flex items-center justify-center shadow-lg"><span class="text-white font-black text-xl">J</span></div>
                ...
                <button id="close-sidebar" class="md:hidden text-white hover:text-accent"><svg class="h-6 w-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18 18 6M6 6l12 12" /></svg></button>
            </div>
        ...
        <main class="flex-1 p-6 md:p-10 overflow-x-auto">
            ...
            <div id="dashboard-view" class="app-view">
                ...
                <section class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div class="lg:col-span-2 bg-white p-6 rounded-xl shadow-lg border border-gray-100"><h2 class="text-xl font-semibold text-primary mb-4">Voter Turnout Ratio</h2><div class="flex justify-center h-80"><canvas id="turnoutChart"></canvas></div></div>
            ...
            <div id="voters-view" class="app-view" style="display:none;">
                <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
                    ...
                    <div class="flex space-x-3 w-full md:w-auto">
                        <button class="flex-1 md:flex-none bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded-lg shadow-md transition-colors duration-200 flex items-center justify-center" onclick="resetAllVotes()"><svg class="h-5 w-5 mr-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0l3.181 3.183a8.25 8.25 0 0013.803-3.7M4.031 9.865a8.25 8.25 0 0113.803-3.7l3.181 3.182m0-4.991v4.99" /></svg>Reset All Votes</button>
                        <button class="flex-1 md:flex-none bg-primary hover:bg-secondary text-white font-bold py-2 px-4 rounded-lg shadow-md transition-colors duration-200 flex items-center justify-center" onclick="openVoterModal()">+ Add Voter</button>
                    </div>
                </div>
                <div class="bg-white rounded-xl shadow-lg overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead><tr><th class="table-header text-center w-16">ID</th><th class="table-header text-left">Student ID</th><th class="table-header text-left">Full Name</th><th class="table-header text-left">Course/Year</th><th class="table-header text-center">Status</th><th class="table-header text-center w-32">Actions</th></tr></thead>
            ...
            <div id="candidates-view" class="app-view" style="display:none;">
                <div class="flex justify-between items-center mb-6"><h2 class="text-2xl font-semibold text-secondary">Registered Candidates</h2><button class="bg-primary hover:bg-secondary text-white font-bold py-2 px-4 rounded-lg shadow-md transition-colors duration-200" onclick="openCandidateModal()">+ Add Candidate</button></div>
                <div class="bg-white rounded-xl shadow-lg overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead><tr><th class="table-header text-center w-16">ID</th><th class="table-header text-center w-24">Profile</th> <th class="table-header text-left">Name</th><th class="table-header text-left">Position</th><th class="table-header text-left">Party</th><th class="table-header text-center w-32">Actions</th></tr></thead>
            ...
            <div id="positions-view" class="app-view" style="display:none;">
                <div class="flex justify-between items-center mb-6"><h2 class="text-2xl font-semibold text-secondary">Electoral Positions</h2><button class="bg-primary hover:bg-secondary text-white font-bold py-2 px-4 rounded-lg shadow-md transition-colors duration-200" onclick="openPositionModal()">+ Add Position</button></div>
                <div class="bg-white rounded-xl shadow-lg overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead><tr><th class="table-header text-center w-16">ID</th><th class="table-header text-left">Position Name</th><th class="table-header text-center">Max Votes</th><th class="table-header text-center w-32">Actions</th></tr></thead>
            ...
            <div id="settings-view" class="app-view" style="display:none;">
                ...
                <div class="bg-white p-6 rounded-xl shadow-lg border border-gray-100 max-w-lg">
                    ...
                    <form id="settings-form" onsubmit="saveElectionSettings(event)">
                        ...
                        <div class="flex justify-end"><button type="submit" id="save-settings-btn" class="bg-primary hover:bg-secondary text-white font-bold py-2 px-6 rounded-lg shadow-md transition-colors duration-200">Save Schedule</button></div>
                    </form>
                </div>
            </div>
    ...
    <script>
        ...
        function updateVotingStatusBadge() {
            ...
            statusDiv.innerHTML = `<span class="px-4 py-1.5 rounded-full text-sm font-extrabold border ${statusColor} flex items-center shadow-inner"><span class="mr-2 h-2 w-2 rounded-full ${statusColor.includes('green') ? 'bg-green-600' : (statusColor.includes('red') ? 'bg-red-600' : 'bg-gray-500')}"></span>${statusText}</span>`;
        }
        ...
        function renderVoters(voterList) {
            ...
            voterList.forEach(voter => {
                ...
                // If is_approved column doesn't exist yet, we treat it as approved (1). Otherwise, strictly check.
                ...
                if (!isApproved) {
                    ...
                    actionBtns = `
                        <button class="text-green-600 hover:text-green-800 font-bold transition-colors" onclick='approveVoter("${voter.id}")'>Approve</button>
                        <button class="text-red-600 hover:text-red-800 font-bold transition-colors ml-2" onclick='deleteItem("voters", "${voter.id}", "${voter.name}")'>Disapprove</button>
                    `;
                } else {
                    ...
                    actionBtns = `
                        ...
                        <button class="text-red-600 hover:text-red-800 font-medium transition-colors ml-2" onclick='deleteItem("voters", "${voter.id}", "${voter.name}")'>Delete</button>
                    `;
                }
        ...
        function renderCandidates(candidateList) {
            ...
            candidateList.forEach(candidate => {
                ...
                tableBody.innerHTML += `<tr class="hover:bg-gray-50 transition-colors"><td class="table-cell text-xs text-gray-400 text-center">${candidate.id}</td><td class="table-cell text-center flex justify-center py-2">${imgTag}</td> <td class="table-cell font-medium text-left">${candidate.name}</td><td class="table-cell font-semibold text-secondary text-left">${position}</td><td class="table-cell text-left">${candidate.party_list || 'Independent'}</td><td class="table-cell space-x-2 text-center"><button class="text-blue-600 hover:text-blue-800 font-medium transition-colors" onclick='openDataModal("candidates", ${JSON.stringify(candidate)})'>Edit</button></td></tr>`;
            });
        }
        ...
        function renderPositions(positionList) {
            ...
            positionList.forEach(position => {
                tableBody.innerHTML += `<tr class="hover:bg-gray-50 transition-colors"><td class="table-cell text-xs text-gray-400 text-center">${position.id}</td><td class="table-cell font-medium text-primary text-left">${position.name}</td><td class="table-cell text-center">${position.maxVotes} vote${position.maxVotes > 1 ? 's' : ''}</td><td class="table-cell space-x-2 text-center"><button class="text-blue-600 hover:text-blue-800 font-medium transition-colors" onclick='openDataModal("positions", ${JSON.stringify(position)})'>Edit</button><button class="text-red-600 hover:text-red-800 font-medium transition-colors" onclick='deleteItem("positions", "${position.id}", "${position.name}")'>Delete</button></td></tr>`;
            });
        }
        ...
        function renderResults() {
            ...
            Object.values(groupedResults).forEach(group => {
                ...
                let candidateRows = candidates.map((c, index) => {
                    ...
                    return `<tr class="${isWinner ? 'bg-accent/30 font-semibold' : index % 2 === 0 ? 'bg-white' : 'bg-gray-50'} transition-all hover:bg-light"><td class="table-cell text-center font-medium">${index + 1}</td><td class="table-cell flex items-center text-left">${avatarHTML} ${c.candidateName}</td><td class="table-cell text-left">${c.partyList || 'Independent'}</td><td class="table-cell text-2xl font-extrabold text-primary text-center">${c.voteCount}</td><td class="table-cell text-sm text-center">${isWinner && c.voteCount > 0 ? '<span class="text-green-600 font-bold">LEADER</span>' : ''}</td></tr>`;
                }).join('');
                resultsContainer.innerHTML += `<div class="bg-white p-6 rounded-xl shadow-lg border border-gray-100"><div class="flex justify-between items-center mb-4 border-b pb-2"><h3 class="text-xl font-bold text-primary">${positionName} (Max: ${maxVotes})</h3><p class="text-lg">${winnerStatus}</p></div><div class="overflow-x-auto"><table class="min-w-full divide-y divide-gray-200"><thead><tr><th class="table-header w-12 text-center">Rank</th><th class="table-header text-left">Candidate</th><th class="table-header text-left">Party List</th><th class="table-header w-24 text-center">Votes</th><th class="table-header w-24 text-center">Status</th></tr></thead><tbody>${candidateRows}</tbody></table></div></div>`;
            });
        }
